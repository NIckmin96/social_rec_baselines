...
def buildSubGraph(mat, subNode):
    ...
    adjMat = sp.dok_matrix((subGraphCount, node_num), dtype=np.int64)
    node_list = []
...
def loadData(datasetStr, seed=None):
    ...
    if seed is None:
        ...
        with open(DIR + f'/test_data_{seed}.pkl', 'rb') as fs:
            testData = pk.load(fs)
        with open(DIR + f'/valid_data_{seed}.pkl', 'rb') as fs:
