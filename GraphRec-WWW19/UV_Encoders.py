...
class UV_Encoder(nn.Module):
    '''
    Docstring for UV_Encoder
    
    1. interaction한 item/user의 list 저장(tmp_history_uv)
    2. 각 item/user list에 대한 rating 저장(tmp_history_r)
    3. UV_Aggregator forward : history의 embedding에 대해서 attention 연산을 적용한 embedding 결과
    4. key user/item embedding에 대한 embedding과 aggregation을 통해 구해진 embedding concatenate
    5. FC layer를 통해서 output 계산
    '''
